...
== NEXT UP TODOS
  validate checkboxes on officer selects in forms
  ...
  ux -> on a users turn (currently always on)
    user will perform province activities and go on to next province once done?
    user will observe provinces from a map and select province, perform actions and indicate when done with their turn?
