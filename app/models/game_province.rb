class GameProvince < ActiveRecord::Base
  ...
  def invest_in_safety options
    options[:generals].each do |general_id|
      general = GameOfficer.find(general_id.to_i)
  ...
  private
    def construction_project options
      ...
      options[:raw_options][:generals].each do |general_id|
        self.send("#{options[:method]}=".to_sym, [100, self.send(options[:method].to_sym) + land_investment_return(gold, GameOfficer.find(general_id))].min)
      end
      self.save!
    end
